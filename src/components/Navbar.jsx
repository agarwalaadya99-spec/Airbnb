import { Search, Globe, Menu, X, UserCircle, Shield, ArrowRight, Home, Layout, Compass, ShieldCheck } from 'lucide-react';
...
const Navbar = () => {
  ...
  const navItems = [
    { to: '/explore', label: 'Explore', icon: Compass, active: !isHost && !isLanding },
    { to: '/host', label: 'Dashboard', icon: Layout, active: isHost }
  ];
        ...
        <div className="max-w-[1440px] mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 lg:gap-10">
            <Link to="/" className="flex items-center gap-1 group shrink-0">
              ...
              <span className="text-2xl font-bold tracking-tight font-manrope text-airbnb hidden xs:block">airbnb</span>
            </Link>
            
            {/* Main Navigation (Context Aware) - Tablet/Desktop */}
              ...
              {navItems.map((item) => (
                <Link 
                  ...
                  className={`flex items-center gap-2 px-6 py-2 rounded-xl text-[14px] font-bold transition-all ${
                    item.active 
                      ? (item.to === '/host' ? 'bg-[#1a1c1c] text-white shadow-md' : 'bg-white text-[#222222] shadow-sm')
          ...
          {/* Global Mode Toggle - Desktop Only */}
          {!isLanding && (
        ...
        {isMenuOpen && (
          <>
            ...
            <motion.div 
              ...
              className="fixed right-0 top-0 bottom-0 z-50 w-[280px] bg-white shadow-2xl p-8 flex flex-col gap-8 md:hidden"
            >
              ...
              <div className="space-y-4">
                <p className="text-[11px] font-black uppercase text-slate-400 tracking-[0.2em] pl-2">Navigation</p>
                <div className="space-y-1">
                  {navItems.map((item) => (
                    <Link 
                      ...
                      to={item.to}
                      onClick={() => setIsMenuOpen(false)}
                      className={`flex items-center gap-4 p-4 rounded-2xl font-bold text-[16px] transition-all ${
                        item.active ? 'bg-airbnb/5 text-airbnb' : 'text-[#1a1c1c] hover:bg-gray-50'
                      }`}
                    >
                  ...
                  >
                    <ShieldCheck size={22} /> Safety Settings
                  </Link>
                </div>
              </div>

              <div className="mt-auto space-y-4">
                 <div className="p-1 bg-gray-50 border border-gray-100 rounded-2xl flex flex-col gap-1">
                    <button 
                      onClick={() => { navigate('/explore'); setIsMenuOpen(false); }}
                      className={`w-full py-4 rounded-xl text-[14px] font-black uppercase tracking-widest transition-all ${!isHost ? 'bg-airbnb text-white shadow-lg' : 'text-slate-400'}`}
                    >
                      Guest Mode
                    </button>
                    <button 
                      onClick={() => { navigate('/host'); setIsMenuOpen(false); }}
                      className={`w-full py-4 rounded-xl text-[14px] font-black uppercase tracking-widest transition-all ${isHost ? 'bg-[#1a1c1c] text-white shadow-lg' : 'text-slate-400'}`}
                    >
                      Host Mode
                    </button>
                 </div>
                 <div className="flex items-center gap-4 p-4 bg-surface-low rounded-[24px]">
                    ...
                    <div>
                       ...
                       <p className="text-[11px] font-bold text-slate-400 uppercase">Pro Host</p>
                    </div>
                 </div>
              </div>
            </motion.div>
